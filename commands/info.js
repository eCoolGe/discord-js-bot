    ...
    async execute(interaction) {
        if (interaction.options.getSubcommand() === 'user') {
            ...
            const userEmbed = new EmbedBuilder()
                ...
                .addFields({ name: '📛' + ' Имя', value: "`" + `${user.username}` + "`", inline: false },
            ...
            const serverEmbed = new EmbedBuilder()
                ...
                .addFields({ name: '🧱' + ' Название', value: "`" + `${server.name}` + "`", inline: false },
                    { name: '🆔' + ' ID', value: "`" + `${server.id}` + "`", inline: false },
                    { name: '💾' + ' Cоздан', value: `<t:` + `${Math.floor(server.createdTimestamp/1000)}` + `:R>`, inline: false },
                    ...
                    { name: '🔑' + ' Канал системных сообщений', value: `${server.systemChannel || '\`-\`'}`, inline: true },
                    { name: '📜' + ' Канал правил сервера', value: `${server.rulesChannel || '\`-\`'}`, inline: true },
